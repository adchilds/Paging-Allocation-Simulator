 ...
 * @since 1.0
 */
public class LRU_Allocation implements PagingAlgorithm {
    ...
    public int[][] retAllocation(int[] references, int frames) {
        ...
        int[][] allocation = allocate_frames();
    ...
    private int[][] allocate_frames() {
        ...
        for (int i=0; i<references.length; i++) {
            ...
            if (i == 0) {
                ...
                // Loop through our previous frames checking for our current page reference.
                // If it is there then there is no page fault and we can continue to the next page reference.
                for (int a=0; a<frames; a++) {
                    if (current_frames[i-1][a] == page_reference) {
                        current_frames[i] = current_frames[i-1];
                        frame_age[a] = 0;
                        log("No page fault occurred, "+page_reference+" is already allocated to frame "+a+".");
                ...
                if (!no_page_fault) {
                    ...
                    // If any equal 0 then set 'empty_frame' to the index and break the loop.
                    ...
                    if (empty_frame != -1) {
                        // We found a free frame to add and its index is stored within 'empty_frame'.
                        System.arraycopy(current_frames[i-1], 0, current_frames[i], 0, frames);
                        current_frames[i][empty_frame] = page_reference;
                        log(page_reference+" was inserted into an empty frame. (frame "+empty_frame+")");
                    } else {
                        // There were no empty frames available, now we must figure out which frame to replace.
                        int max_age = 0;
                        int replace_frame = 0;

                        // Find the frame that hasn't been used in the longest time.
                        ...
                        // Replace that frame with the new page reference and set it's age to 0.
                        log("Frame "+replace_frame+" is least recently used and should be replaced.");
                        System.arraycopy(current_frames[i-1], 0, current_frames[i], 0, frames);
                        current_frames[i][replace_frame] = page_reference;
                        frame_age[replace_frame] = 0;
                        log(current_frames[i-1][replace_frame]+" was replaced by "+page_reference+" at frame "+replace_frame);
                    }
                    page_faults++;
                }
            }

            // Iterate through current_frames and increment all necessary frame ages.
            ...
            if (DEBUG_MODE) {
                ...
                for (int h=0; h<frames; h++) {
                    log("Frame "+h+": "+current_frames[i][h]+" => age: "+frame_age[h]);
                }
                log("---- End Current Frames ----");
                log("");
            }
        }
