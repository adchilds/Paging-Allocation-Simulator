...
/**
 * This class handles adding data to the JTables. We use this class because it allows us to run data insertions for
 * each {@link JTable} on a separate thread, independent of the main program thread. This way, we can modify
 * the delay by calling the {@link Thread#sleep(long)} method. We allow the user to set the delay so that they can see,
 * in real time, the data being added to the table.
 *
 * @author Adam Childs
 * @since 1.0
 */
public class TableInsertion extends Thread {
    ...
    /**
     * Moves the table's scrollbar to show the current column and row having data added to it.
     */
